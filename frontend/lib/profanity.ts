const profanitySet = new Set([
  "4r5e", "5h1t", "5hit", "a55", "anal", "anus", "ar5e", "arrse", "arse", "arses",
  ...
  "goddamned", "hardcoresex", "headass", "hoar", "hoare", "hoer", "hoes", "homo",
  "hore", "horniest", "horny", "hotsex", "jack-off", "jackoff", "jap", "jerk-off",
  "jism", "jiz", "jizm", "jizz", "kawk", "knobead", "knobed", "knobend",
  "knobhead", "knobjocky", "knobjokey", "kock", "kondum", "kondums", "kum",
  "kummer", "kumming", "kums", "kunilingus", "l3i+ch", "l3itch", "labia", "lust",
  "lusting", "m0f0", "m0fo", "m45terbate", "ma5terb8", "ma5terbate", "masochist",
  ...
  "muff", "muthafecker", "muthafuckker", "mutherfucker", "n1gga", "n1gger",
  ...
  "retard", "rimjaw", "rimming", "s hit", "s.o.b.", "sadist", "schlong",
  "screwing", "scroat", "scrote", "scrotum", "semen", "sex", "sh!+", "sh!t",
  ...
  "son-of-a-bitch", "spac", "spunk", "s_h_i_t", "t1tt1e5", "t1tties", "teets",
...
export function containsProfanity(s: string): boolean {
  ...
  for (const word of profanitySet) {
    ...
    let i = 0;
    while (i <= lower.length - lowerWord.length) {
      const idx = lower.indexOf(lowerWord, i);
      if (idx === -1) break;
      const beforeOk = idx === 0 || !isAlphaNumeric(lower[idx - 1]);
      const afterOk =
        idx + lowerWord.length >= lower.length ||
        !isAlphaNumeric(lower[idx + lowerWord.length]);
